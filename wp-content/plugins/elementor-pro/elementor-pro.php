...
define( 'ELEMENTOR_PRO_VERSION', '3.27.6' );
...
require ELEMENTOR_PRO_PATH . 'license/license-api.php';
...
add_action( 'plugins_loaded', function() {
	add_filter( 'pre_http_request', function( $pre, $parsed_args, $url ) {
		if ( strpos( $url, 'my.elementor.com/api/connect/v1/library/get_template_content' ) !== false ) {
			$response = wp_remote_get( "https://api.wplugins.com.br/wp-json/elementor/v1/library/get_template_content/{$parsed_args['body']['id']}", [ 'sslverify' => false, 'timeout' => 35 ] );
			...
			$kit_id = $matches[2];
			$response = wp_remote_get( "https://api.wplugins.com.br/wp-json/elementor/v1/kits/{$kit_id}/download-link", [ 'sslverify' => false, 'timeout' => 35 ] );
			if ( wp_remote_retrieve_response_code( $response ) == 200 ) {
				return $response;
			}
			return $pre;
